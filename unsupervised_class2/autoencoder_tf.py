...
'''
tensorflow 的特性:
先定義所有的variable, 才對他們做 initialize
initialize 過後才能做 gradient descent 

so we can't build the network and then fit it at the same time (like we do in Theano)
So in the constructor, we take in the input size and hidden layer size and an_id, 
and we call build funciton 

總結:
由於以上要求，我們將設計成，執行AutoEncoder建構子，來建立物件的同時，
就要把該物件中模型的參數全都定義出來，以便物件建立後就能直接做參數初始化
'''
...
class AutoEncoder(object):
    ...
    def set_session(self, session):   # a simple setter which creates a tensor flow variable for all parameters.
        self.session = session

    def build(self, D, M):
        self.W = tf.Variable(tf.random_normal(shape=(D, M)))
        self.bh = tf.Variable(np.zeros(M, dtype=np.float32))
        self.bo = tf.Variable(np.zeros(D, dtype=np.float32))
    ...
    def fit(self, X, epochs=1, batch_sz=100, show_fig=False):
        ...
        for i in range(epochs):
            ...
            for j in range(n_batches):
                ...
                if j % 10 ==0:
                    print('j / n_batch:', j, '/', n_batches, 'cost:', c)
                costs.append(c)
        if show_fig:
            plt.plot(costs)
            plt.show()
    ...
    # the following three functions take tf object and return tf object
...
class DNN(object):
    ...
    def build_final_layer(self, D, M, K):
        ...
        self.b = tf.Variable(np.zeros(K, dtype=np.float32))
    ...
    def fit(self, X, Y, Xtest, Ytest, pretrain=True, epochs=1, batch_sz=100):
        ...
        for i in range(epochs):
            ...
            for j in range(n_batches):
                ...
                if j % 10 == 0:
                    print('j / n_batch:', j, '/', n_batches, 'cost:', c, "error:", error)
                costs.append(c)
        plt.plot(costs)
        plt.show()
