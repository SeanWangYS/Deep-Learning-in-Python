...
def main():
    ...
    for i in range(50): # takes very long since we're computing cost for 41k samples
        ...
        for n in range(min(N, 500)): # shortcut so it won't take so long...
            x = tmpX[n,:].reshape(1,D)      # in this case, x is a vector, shape=(D,) ，為了讓 feature and target 可以計算 forward and Weights 要做reshape
            y = tmpY[n,:].reshape(1,10)     # y is a vector, need to convert into metrix for y2indicator calculation 
            p_y = forward(x, W, b)
