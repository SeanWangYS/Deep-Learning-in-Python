...
class HiddenLayer(object):
    def __init__(self, M1, M2, an_id):
        ...
        W = init_weight(M1, M2)
        b = np.zeros(M2)
        self.W = theano.shared(W, 'W_%s' % self.an_id)
        self.b = theano.shared(b, 'b_%s' % self.an_id)
...
class ANN(object):
    ...
    def fit(self, X, Y, learning_rate=1e-2, mu=0.9, decay=0.999, reg=1e-12, eps=10e-10, epochs=400, batch_sz=20, print_periods=1, show_fig=False):
...
def deep():
    ...
    model = ANN([1000]*2)
    model.fit(X, Y, learning_rate=1e-3, print_periods=10, epochs=100, show_fig=True)
